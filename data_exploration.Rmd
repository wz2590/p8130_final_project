...
=======
library(MASS)
>>>>>>> b28dc3f3d752e02a07ecff69018d992e62003d37
...
mult.fit = lm(CRM_1000 ~ ., data = new_cdi)
summary(mult.fit)
```
...
```{r}
our_new_model = lm(CRM_1000 ~ id + pop + pop18 + docs + crimes + poverty + unemp + pcincome, data = new_cdi)
...
mod_df = 
  new_cdi %>% 
  select(CRM_1000, id , pop , pop18 , docs , crimes , poverty , unemp , pcincome)
...
cv_mod = 
  cv_mod %>% 
  mutate(
    model_1  = map(train, ~lm(CRM_1000 ~ id + pop + pop18 + docs + crimes + poverty + unemp + pcincome, data = .x))) %>%
  mutate(
